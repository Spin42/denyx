...
| **Agent exfiltrating local-only secrets to a cloud orchestrator via printed output** | `[environment].local_only_vars`, `[filesystem].local_only_read`, `[network].local_only_hosts`, and `[subprocess].local_only_commands` mark values as tainted. Four layers of defense, in pre-exec → runtime order: (1) **pre-exec tainted-output-flow refusal** in the verifier — a script that contains both a literal-arg `env.read`/`fs.read` of a local-only value AND any output-producing call (`print`, `fs.write`, `fs.delete`, `net.http_*`, `subprocess.exec`) is rejected before the Starlark evaluator runs (Round-2 pentest 2026-05-11 — 63/64 step-injection probes across 4 local models held here, 0 leaks); (2) substring scrub against the value AND the documented transform set (reverse, hex lower/upper, XOR with each single-byte key, hex(XOR) compositions, base64 std + url-safe ± padding, ROT-1..25) — the runtime fallback for the variable-arg case the static analysis cannot resolve; (3) subsequence-chunking detection on the joined printed output (catches per-character prints interleaved with cover text); (4) arg-side denial at every outbound effect (`fs.write`, `fs.delete`, `subprocess.exec` to non-local-only commands, `net.http_*` to non-local-only hosts) — the call is *refused* with a typed error if any string argument carries tainted bytes in any covered form. **Limited — see "Known limitations" below.** |
...
| **MCP tool definition poisoning (local-executor deployment)** | In the `denyx-local-mcp` architecture, the cloud orchestrator sees exactly one tool — `delegate_to_local` — so third-party MCP tool descriptions never enter its context. **Precondition:** the MCP host must be launched with `--strict-mcp-config` (or equivalent) and `denyx-local-mcp` must be the only MCP server configured for the project. Without that precondition, the cloud model reads all co-installed servers' tool descriptions and the structural claim fails. The local executor model receives tool metadata only from the operator-controlled policy file (`[tools.X]` routing entries), not from live MCP server schemas. A poisoned tool description in any co-installed MCP server has no path to either model's reasoning **under the precondition**. See [Round 2 pentest report](security-pentest-r2-tool-poisoning.md) for the validation. |
| **Step-parameter injection from a (possibly poisoned) cloud orchestrator into the local executor** | The step text reaching `delegate_to_local` is treated as untrusted by the system prompt and wrapped in `<task>…</task>` delimiters. Empirically (Round 2 pentest, 4 local models 7B–22B), these wrappers do **not** prevent prompt injection at the LLM layer — the local model obeys the injected instructions ~100% of the time. The defense that actually holds is the **pre-exec tainted-output-flow refusal** + the IFC + arg-side gate + policy gate stack: 63/64 step-injection probes across 4 models were refused before evaluation; the remaining 1 was vacuously safe (read into a variable, never printed). The `<task>` defenses are documented as mitigations, not security boundaries. |
| **Reads of resources the policy doesn't mention** | Default-deny: `fs.read` of a path not in `read_allow` or `local_only_read` returns a typed `PolicyError::PathDenied`; `env.read` of a name not in `allow_vars` or `local_only_vars` returns `PolicyError::EnvDenied`; `subprocess.exec` of a command not in `allow_commands` or `local_only_commands` returns `PolicyError::SubprocessDenied`. **Operator caveat:** `read_allow = ["**"]` (or a too-broad rooted glob) defeats deny-by-default by construction. The narrowest set of allowed paths is the operator's responsibility. |
