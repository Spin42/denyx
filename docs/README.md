...
| [security-pentest-r3-argv0-and-chunking](security-pentest-r3-argv0-and-chunking.md) | Round 3 (2026-07-02): Sonnet 5 + Fable 5 white-box pentest. One Critical finding (`subprocess.exec` argv[0] basename-only matching → arbitrary code execution) and three High findings (chunk-a-transform, chunking-density dilution, deny/allow basename asymmetry), all remediated. Also added a new `denyx doctor` check for local-executor MCP isolation. |
| [security-pentest-r4-wasm-path-regressions](security-pentest-r4-wasm-path-regressions.md) | Round 4 (2026-07-03): Sonnet 5 structural review + independent Fable 5 pentest of the wasm sandbox (the default execution path since v0.4.0). Two High findings (hostname-based `deny_ips` unenforced, `sandbox = "bwrap"` unenforced) and two Medium findings (no HTTP timeout / blocking-I/O hang, guest-length host allocation before bounds-check), all remediated. |
...
| [landlock-evaluation](landlock-evaluation.md)                | `[subprocess].sandbox = "landlock"`: a second, unprivileged OS-level sandbox backend for environments where bwrap's user-namespace requirement isn't available. What it does and doesn't provide vs. bwrap, real friction found while implementing it, verification against a real kernel. |
