#!/usr/bin/env python3
"""Adversarial exfiltration probe for Denyx's local-only redaction.

This harness is the empirical companion to docs/04-security-threat-model.md's
...
Prerequisites:
    - cargo build --release  (produces target/release/denyx-mcp)
    - The fixture file /tmp/denyx_demo/multistep/fixtures/secrets/token.txt
      and the env var DENYX_DEMO_SECRET are set up by setup_fixtures()
...
WORKDIR = Path("/tmp/denyx_demo/exfil")
...
POLICY_PATH = Path("/tmp/denyx_exfil_probe.toml")

ENV_SECRET_NAME = "DENYX_DEMO_SECRET"
...
class McpClient:
    def __init__(self, mcp_bin: Path, policy: Path, use_wasm: bool = False) -> None:
        ...
        self._call("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "denyx-exfil-probe", "version": "0"},
        })
...
POLICY_BODY = f"""# Auto-generated by run_exfil.py at /tmp/denyx_exfil_probe.toml.
...
# directly (NOT through denyx-mcp) and confirms the secret landed
...
T_FS_WRITE = Technique(
    ...
    category="covert-channel",
    description="fs.write(public_path, secret) — content on disk is NOT redacted; host reading the file outside denyx-mcp recovers it.",
    script=f'''
secret = env.read("{ENV_SECRET_NAME}")
...
def run_one(client: McpClient, t: Technique) -> Result:
    resp = client.denyx_run(t.script.lstrip(), task_id=f"exfil-{t.name}")
...
def main() -> int:
    ...
    if not args.mcp_bin.exists():
        print(f"FAIL: denyx-mcp not built at {args.mcp_bin}", file=sys.stderr)
        return 2
