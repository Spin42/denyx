...
mod tests {
    ...
    #[test]
    fn strips_string_literal() {
        let stripped = strip_strings_and_comments(r#"x = "fs_read"
y = 1"#);
        assert!(!stripped.contains("fs_read"));
    }

    #[test]
    fn strips_line_comment() {
        let stripped = strip_strings_and_comments("# fs_read is dangerous\nfs_write(\"x\", \"y\")");
        assert!(!stripped.contains("fs_read"));
        assert!(stripped.contains("fs_write"));
    }

    #[test]
    fn strips_triple_quote() {
        let stripped = strip_strings_and_comments(r#"x = """
this docstring mentions fs_read
"""
fs_write("x", "y")"#);
        assert!(!stripped.contains("fs_read"));
        assert!(stripped.contains("fs_write"));
