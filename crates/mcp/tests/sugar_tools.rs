...
//!
//! `aegis_run` (the primary tool) is heavily exercised by the
//! orchestrated harness, the elicitation tests, and the server-mode
//! tests. The sugar variants — `aegis_fs_read`, `aegis_fs_write`,
//! `aegis_fs_delete`, `aegis_subprocess_exec`, `aegis_net_http_get`,
//! `aegis_net_http_post`, `aegis_env_read` — are part of the MCP
...
fn sugar_fs_read_returns_file_contents() {
    let dir = std::env::temp_dir().join(format!("aegis_sugar_read_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
"#
    ...
    s.send(&json!({
        ...
        "params": {"name": "aegis_fs_read", "arguments": {"path": target.to_string_lossy()}}
    }));
    let resp = s.recv();
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_fs_read", "arguments": {"path": "/etc/passwd"}}
    }));
    let resp = s.recv();
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_fs_read", "arguments": {}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], true, "{resp}");
...
fn sugar_fs_write_creates_file() {
    let dir = std::env::temp_dir().join(format!("aegis_sugar_write_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
"#
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_fs_write", "arguments": {
...
fn sugar_fs_delete_removes_file() {
    let dir = std::env::temp_dir().join(format!("aegis_sugar_delete_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
"#
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_fs_delete", "arguments": {"path": target.to_string_lossy()}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], false, "{resp}");
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_subprocess_exec", "arguments": {
            "argv": ["echo", "from-aegis-mcp-test"]
        }}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], false, "{resp}");
    let text = resp["result"]["content"][0]["text"].as_str().unwrap_or("");
    assert!(text.contains("from-aegis-mcp-test"), "got: {text}");
    s.close();
    let _ = std::fs::remove_dir_all(&pdir);
}
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_subprocess_exec", "arguments": {"argv": []}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], true, "{resp}");
    s.close();
...
fn sugar_env_read_returns_value_for_allowed_var() {
    let var_name = format!("AEGIS_SUGAR_TEST_{}", std::process::id());
...
"#
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_env_read", "arguments": {"name": var_name}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], false, "{resp}");
    let text = resp["result"]["content"][0]["text"].as_str().unwrap_or("");
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_env_read", "arguments": {"name": "SOME_OTHER_VAR"}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], true, "{resp}");
    s.close();
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_net_http_get", "arguments": {"url": "https://example.com/"}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], true, "{resp}");
    s.close();
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_net_http_post", "arguments": {
...
"#,
    ...
    s.send(&json!({
        "jsonrpc": "2.0", "id": 2, "method": "tools/call",
        "params": {"name": "aegis_does_not_exist", "arguments": {}}
    }));
    let resp = s.recv();
    assert_eq!(resp["result"]["isError"], true, "{resp}");
    let text = resp["result"]["content"][0]["text"].as_str().unwrap_or("");
