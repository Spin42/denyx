...
# Crates that must be published with `--allow-dirty`.
# `denyx-runtime-starlark` bundles `starlark_interpreter.wasm`,
# which is generated by `scripts/build-runtime-starlark.sh` and
# intentionally gitignored. Cargo treats the resulting untracked
# file as a dirty working directory and refuses to publish without
# `--allow-dirty`. Scope the flag to this crate so the safety
# check still applies to the other five.
DIRTY_CRATES=(
    denyx-runtime-starlark
)
